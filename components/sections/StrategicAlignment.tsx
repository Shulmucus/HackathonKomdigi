...
export function StrategicAlignment() {
  ...
  return (
    <section className="py-16 lg:py-24 relative overflow-hidden bg-primary-700">
      {/* Geometric Decorations */}
      <div className="absolute top-20 -left-20 w-[400px] h-[400px] bg-white/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          {themes.map((theme, i) => (
            ...
            >
              <Image 
                ...
                className="mb-6"
              />
              <h3 className="text-2xl font-bold text-white/60 mb-6 tracking-wide">{theme.title}</h3>
              <div className="text-left space-y-4 inline-block px-4 w-full">
                {theme.items.map((item, j) => {
                  ...
                  return (
                    <div key={itemKey} className="rounded-3xl border border-white/20 overflow-hidden bg-white/10 shadow-sm">
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`w-5 h-5 transition-transform duration-200 ${isOpen ? "rotate-180 text-accent-gold" : "text-white/50"}`}
                        />
                      </button>
                      ...
                      <AnimatePresence initial={false}>
                        {isOpen && item.subitems && item.subitems.length > 0 && (
                          ...
                          >
                            <div className="px-5 pb-4 pt-2 bg-white/5">
        ...
        <motion.div
          ...
          className="mt-12 mx-auto max-w-full bg-white/10 rounded-3xl overflow-hidden"
        >
          <button
            type="button"
            onClick={() => toggleItem("dukungan-manajemen")}
            className="w-full flex items-center justify-between gap-4 px-10 py-8 text-left hover:bg-white/10 transition-colors"
          >
            <h3 className="text-lg font-extrabold text-white tracking-wide">
              DUKUNGAN MANAJEMEN
            </h3>
            <ChevronDown
              className={`w-6 h-6 flex-shrink-0 transition-transform duration-200 ${openItems["dukungan-manajemen"] ? "rotate-180 text-accent-gold" : "text-white/50"}`}
            />
          </button>

          <AnimatePresence initial={false}>
            {openItems["dukungan-manajemen"] && (
              <motion.div
                initial={{ height: 0, opacity: 0 }}
                animate={{ height: "auto", opacity: 1 }}
                exit={{ height: 0, opacity: 0 }}
              ...
              >
                <div className="px-10 pb-8 pt-0">
                  <p className="text-white/80 text-[15px] md:text-xl leading-relaxed text-center">
